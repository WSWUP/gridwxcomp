...
# Start Date, End Date (Inclusive), and List for File Naming
...
for date in date_list:
    ...
    gridmet_coll = ee.ImageCollection('IDAHO_EPSCOR/GRIDMET') \
        .filterDate(date, date+1)

    gridmet_etr = gridmet_coll.map(gridmet_etr_func)

    gridmet_image = ee.Image(gridmet_etr.first())

    ee.batch.Export.image.toCloudStorage(
        image=gridmet_image,
        bucket='ee_exports',
        fileNamePrefix='{}/{}_{}'.format('ETr_Daily', 'ETr', date.strftime('%Y%m%d')),
        dimensions='1386x585',
        crs='EPSG:4326',
